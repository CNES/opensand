...
class SpotGwAssignmentDialog(WindowView):
    ...
    def load(self):
        ...
        for spot_id in self._spot_list:
            ...
            if spot_id not in spot_list:
                ...
                terminal_path = PATH_SPOT + "/spot[@id='" + spot_id + "']/terminals"
        ...
        for gw in config.get_table_elements(config.get(PATH_GW)):
            if gw.tag == "gw" : 
                ...
                if (int(content[ID])) not in gw_list:
                    ...
                    terminal_path = PATH_GW + "/gw[@id='" + content[ID] + "']/terminals"
        ...
        for host in host_list:
            ...
            def_spot = self.get_spot_value(spot_list,
                                           int(self.get_default_spot_assignment()))
    ...
    def get_default_gw_assignment(self):
        ...
        self._defaulf_gw = int(config.get_value(config.get(defaulf_gw_path)))
        return self._defaulf_gw
            
    ##################################################
    
    def get_spot_value(self, list_spot, spot_id):
    ...
    def on_save_edit_clicked(self, source=None):
        ...
        spot_list = []
        for st in st_list:
            if st[1] not in nb_line_spot:
                nb_line_spot[st[1]] = 1
            else:
                nb_line_spot[st[1]] += 1
            
            if st[2] not in nb_line_gw:
                nb_line_gw[st[2]] = 1
            else:
                nb_line_gw[st[2]] += 1

            cmp_line_spot[st[1]] = 0
            cmp_line_gw[st[2]] = 0
        ...
        for st in st_list:
            ...
            #     SPOT    #
            # ------------#
            spot_path = PATH_SPOT + "/spot[@id='" + self._spot_list[st[1]] + "']"
            spot = topo.get(spot_path)
            
            # add not existing spot
            if spot is None and self._spot_list[st[1]] not in spot_list:
                manager.add_spot(self._spot_list[st[1]])        
            
            xpath = spot_path + "/terminals"
            table = topo.get_table_elements(topo.get(xpath))
            
            # remove terminal's line 
            if len(table) > nb_line_spot[st[1]]:
                nb_remove = len(table) - nb_line_spot[st[1]]
                topo.remove_line(xpath , nb_remove)
            # add terminal's line
            elif len(table) < nb_line_spot[st[1]]:
                nb_add = nb_line_spot[st[1]] - len(table)
            ...
            topo.set_value(self.get_id_st(st[0]),
                             topo.get_path(topo.get_table_elements(table)[cmp_line_spot[st[1]]]),
                             ID)
            cmp_line_spot[st[1]] += 1 
            spot_list.append(self._spot_list[st[1]])
            ...
            # remove terminal's line 
            if len(table) > nb_line_gw[st[2]]:
                nb_remove = len(table) - nb_line_gw[st[2]]
                topo.remove_line(terminal_path , nb_remove)
            # add terminal's line
            elif len(table) < nb_line_gw[st[2]]:
                nb_add = nb_line_gw[st[2]] - len(table)
                for i in range(nb_add):
