 ...
 */
class interconnect_channel_test
{
 public:

	interconnect_channel_test(bool input, bool output);
	~interconnect_channel_test();
	...
	/**
	 * @brief Send packet via the TCP socket
	 *
	...
	*/
     ...
     */
    void flush() {this->send((unsigned char *) NULL, 0);};
 ...
 private:
    ...
    unsigned char send_buffer[10*MAX_SOCK_SIZE];
    ...
    unsigned char recv_buffer[10*MAX_SOCK_SIZE];
