...
bool BlockDvbSat::Upward::onInit()
{
	...
	// TODO once all DVB blocks will be correctly splitted, move
	//      into Upward::initCommon
	this->with_phy_layer = ((BlockDvb *)this->block)->with_phy_layer;
	this->up_return_pkt_hdl = ((BlockDvb *)this->block)->up_return_pkt_hdl;
...
bool BlockDvbSat::Upward::initMode(void)
{
	...
	UTI_INFO("Satellite delay = %d", this->sat_delay);
...
bool BlockDvbSat::Downward::onInit()
{
	...
	if(!this->initSatLink())
	{
		UTI_ERROR("failed to complete the initialisation of "
		          "link parameters");
		return false;
	}
	...
	if(this->satellite_type == REGENERATIVE)
	{
		if(!((BlockDvb *)this->block)->initForwardModcodFiles())
		{
	...
	if(!this->initTimers())
	{
		UTI_ERROR("failed to initialize timers");
		return false;
	}
...
bool BlockDvbSat::Downward::initStList(void)
{
	...
	for(iter = column_list.begin(); iter != column_list.end(); iter++)
	{
		...
		if(!this->fwd_fmt_simu.doTerminalExist(tal_id))
		{
			...
			{
				UTI_ERROR("failed to register ST with Tal ID %u\n", tal_id);
				goto error;
			}
		}
	}
...
{
	// TODO split in upward and downard parts
...
bool BlockDvbSat::Downward::onEvent(const RtEvent *const event)
{
	switch(event->getType())
	{
		case evt_message:
		{
			if(((MessageEvent *)event)->getMessageType() == MSG_TYPE_CNI)
			{
				cni_info_t *info = (cni_info_t *)((MessageEvent *)event)->getData();
				this->fwd_fmt_simu.setRequiredModcod(info->tal_id,
				                                     info->cni);
				delete info;
				break;
			}
			...
			for(spot = this->spots.begin(); spot != this->spots.end(); spot++)
			{
				...
				if(current_spot->getInputCarrierId() == dvb_frame->getCarrierId())
				{
					...
					UTI_DEBUG("DVB burst comes from spot %u (carrier %u) => "
					          ...
					          current_spot->getSpotId(),
					          current_spot->getDataOutGwFifo()->getCarrierId());
	...
	case MSG_TYPE_SAC:
		if(this->with_phy_layer && this->satellite_type == REGENERATIVE)
		{
			...
			if(!this->shareMessage((void **)&cni_info, sizeof(cni_info_t),
			                       MSG_TYPE_CNI))
			{
				UTI_ERROR("Unable to transmit downward CNI to channel\n");
			}
...
bool BlockDvbSat::Upward::forwardDvbFrame(DvbFifo *fifo, DvbFrame *dvb_frame)
{
	...
	if(!elem)
	{
		UTI_ERROR("failed to create a MAC FIFO element, drop the frame\n");
		goto error;
	}
	...
	if(!fifo->push(elem))
	{
		UTI_ERROR("FIFO %s full, drop the DVB frame\n",
		          fifo->getName().c_str());
		goto release_elem;
	}

	UTI_DEBUG("frame stored in FIFO for carrier ID %d "
...
bool BlockDvbSat::Downward::sendFrames(DvbFifo *fifo)
{
	      ...
	      fifo->getCurrentSize() > 0)
	{
		...
		UTI_DEBUG("Frame sent with a size of %zu\n",
		          dvb_frame->getTotalLength());
