...
BlockDvbNcc::Downward::~Downward()
{
	...
	{
		delete this->spot_downward_map[i];
	}
}
...
//	this->addNetSocketEvent("pep_listen", this->getPepListenSocket(), 200);
	...
	for(int i = 0; i < 2; i++)
	{
		this->spot_downward_map[i] = new SpotDownward(this->fwd_down_frame_duration_ms,
												...
												this->pkt_hdl,
												this->with_phy_layer);      
		this->spot_downward_map[i]->setSpotId(i);
		result = this->spot_downward_map[i]->onInit();
	}
...
bool BlockDvbNcc::Downward::onEvent(const RtEvent *const event)
{
	...
	switch(event->getType())
	{
		case evt_message:
		{
			...
			spot_id_t dest_spot = dvb_frame->getDstSpot();
			...
			if(this->spot_downward_map[dest_spot] != NULL)
			{
				...
				if(((MessageEvent *)event)->getMessageType() == msg_sig)
				{
					...
					switch(msg_type)
					{
						...
						case MSG_TYPE_CORRUPTED:
							{
								...
								else
								{
									...
									if(!this->spot_downward_map[dest_spot]->getUpReturnPktHdl()->getSrc(frame->getPayload(), tal_id))
									{
						...
						case MSG_TYPE_SESSION_LOGON_REQ:
							if(!this->spot_downward_map[dest_spot]->handleLogonReq(dvb_frame, &logonResp, ctrlCarrierId, this->super_frame_counter))
							{
								goto error;
							}
						...
						case MSG_TYPE_SESSION_LOGOFF:
							if(!this->spot_downward_map[dest_spot]->handleLogoffReq(dvb_frame, this->super_frame_counter))
							{
								goto error;
							}
							break;
				...
				{
					this->spot_downward_map[dest_spot]->onEvent(event, this->super_frame_counter);
				}
				break;
			}   
			break;
		...
		case evt_timer:
		{
			...
			if(*event == this->frame_timer)
			{
				...
				// for each spot
				for(size_t i = 0 ; i < this->spot_downward_map.size(); i++)
				{
					// sendSOF appelé dans spot (idem senTTP)
					// send Start Of Frame (Spot)
					this->sendSOF(spot_downward_map[i]);


					// fonction sur chaque spot
					if(!this->spot_downward_map[i]->getDamaCtrl())
					{
						// stop here
						return true;
					}
					...
					if(this->with_phy_layer)
					{
						...
						this->spot_downward_map[i]->getDamaCtrl()->updateFmt();
					}

					// run the allocation algorithms (DAMA)
					this->spot_downward_map[i]->getDamaCtrl()->runOnSuperFrameChange(this->super_frame_counter);
					// send TTP computed by DAMA
					this->sendTTP(spot_downward_map[i]);

					// **************
					// Simulation
					// **************
					switch(this->spot_downward_map[i]->getSimulate())
					{
						case file_simu:
							if(!this->spot_downward_map[i]->getSimuFile())
							{
								LOG(spot_downward_map[i]->getLogRequestSimulation(), LEVEL_ERROR,
										"file simulation failed");
								fclose(this->spot_downward_map[i]->getSimuFile());
								this->spot_downward_map[i]->setSimuFile(NULL);
								this->spot_downward_map[i]->setSimulate(none_simu);
							}
							break;
						case random_simu:
							this->spot_downward_map[i]->simulateRandom();
							break;
						default:
							break;
					}
					// flush files
					fflush(this->spot_downward_map[i]->getEventFile());
			...
			else if(*event == this->fwd_timer)
			{
				...
				for(size_t i = 0 ; i < this->spot_downward_map.size() ; i++)
				{
					this->spot_downward_map[i]->updateStatistics();
					this->spot_downward_map[i]->setFwdFrameCounter(this->spot_downward_map[i]->getFwdFrameCounter()+1);
					...
					if(!this->spot_downward_map[i]->schedule(
								getCurrentTime(),
								remaining_alloc_sym))
					{
						LOG(this->log_receive, LEVEL_ERROR,
					...
					if(this->satellite_type == REGENERATIVE &&
							spot_downward_map[i]->getCompleteDvbFrames().size() > 0)
					{
						...
						uint8_t modcod_id;
						modcod_id =
							((DvbRcsFrame *)this->spot_downward_map[i]->getCompleteDvbFrames().front())->getModcodId();
						this->spot_downward_map[i]->getProbeUsedModcod()->put(modcod_id);
					}
					...
					if(!this->sendBursts(&this->spot_downward_map[i]->getCompleteDvbFrames(),
								spot_downward_map[i]->getDataCarrierId()))
					{
						LOG(this->log_receive, LEVEL_ERROR,
			...
			else if(*event == this->scenario_timer)
			{
				...
				for(size_t i = 0 ; i < this->spot_downward_map.size() ; i++)
				{
					...
					if(this->satellite_type == REGENERATIVE &&
							this->with_phy_layer)
					{
						this->sendAcmParameters(spot_downward_map[i]);
					}
					...
					if(spot_downward_map[i]->getDamaCtrl())
					{
						...
						spot_downward_map[i]->getDamaCtrl()->updateFmt();
					}
				}
			}
...
bool BlockDvbNcc::Upward::onInit(void)
{
	...
	for(spot_id_t i = 0; i < 2; i++)
	{
		this->spot_upward_map[i] = new SpotUpward();
		this->spot_upward_map[i]->setSpotId(i);
		
		result = this->spot_upward_map[i]->onInit();
	}
		...
		case evt_message:
			{
				...
				//	"upward  dest_spot :%d set to 0 msg: %d", dest_spot, dvb_frame->getMessageType());
					...
					case MSG_TYPE_CORRUPTED:
						{
							...
							// Update stats
							this->spot_upward_map[dest_spot]->setL2FromSatBytes(this->spot_upward_map[dest_spot]->getL2FromSatBytes() + dvb_frame->getPayloadLength());
							...
							if(!this->spot_upward_map[dest_spot]->getReceptionStd()->onRcvFrame(dvb_frame,
