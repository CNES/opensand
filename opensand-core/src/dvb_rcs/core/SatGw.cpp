...
bool SatGw::initProbes(sat_type_t satellite_type)
{
	...
	probe_output_st = Output::registerProbe<int>(
			...
			"Spot_%d.Gw_%d.Delay buffer size.Output_ST", this->spot_id, this->gw_id);
	this->probe_sat_output_st_queue_size.insert(
			std::pair<unsigned int, Probe<int> *>(this->gw_id, probe_output_st));

	probe_output_st_kb = Output::registerProbe<int>(
			"Kbits", false, SAMPLE_LAST,
			"Spot_%d.Gw_%d.Delay buffer size.Output_ST_kb", this->spot_id, this->gw_id);
	this->probe_sat_output_st_queue_size_kb.insert(
			std::pair<unsigned int, Probe<int> *>(this->gw_id, probe_output_st_kb));
	...
	this->probe_sat_l2_from_st.insert(
			std::pair<unsigned int, Probe<int> *>(this->gw_id, probe_l2_from_st));
	...
	if(satellite_type == TRANSPARENT)
	{
		...
		probe_l2_to_gw = Output::registerProbe<int>(
				"Kbits/s", true, SAMPLE_LAST, "Spot_%d.Gw_%d.Throughputs.L2_to_GW",
				this->spot_id, this->gw_id);
		this->probe_sat_l2_to_gw.insert(
				std::pair<unsigned int, Probe<int> *>(this->gw_id, probe_l2_to_gw));
		probe_l2_from_gw = Output::registerProbe<int>(
				"Kbits/s", true, SAMPLE_LAST, "Spot_%d.Gw_%d.Throughputs.L2_from_GW",
				this->spot_id, this->gw_id);
		this->probe_sat_l2_from_gw.insert(
				std::pair<unsigned int, Probe<int> *>(this->gw_id, probe_l2_from_gw));
		probe_output_gw = Output::registerProbe<int>(
				"Packets", false, SAMPLE_LAST,
				"Spot_%d.Gw_%d.Delay buffer size.Output_GW", this->spot_id, this->gw_id);
		this->probe_sat_output_gw_queue_size.insert(
				std::pair<unsigned int, Probe<int> *>(this->gw_id, probe_output_gw));

		probe_output_gw_kb = Output::registerProbe<int>(
				"Kbits", false, SAMPLE_LAST,
				"Spot_%d.Gw_%d.Delay buffer size.Output_GW_kb", this->spot_id, this->gw_id);
		this->probe_sat_output_gw_queue_size_kb.insert(
				std::pair<unsigned int, Probe<int> *>(this->gw_id, probe_output_gw_kb));
	}
...
uint8_t SatGw::getInputCarrierId(void) const
{
