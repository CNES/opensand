...
bool Ip::Context::onMsgIp(IpPacket *ip_packet)
{
	...
	// set QoS:
        ...
        {
                UTI_DEBUG("IP packet with DSCP %d goes to MAC category %d\n", traffic_category, found_category->second->ip);
        }
        ip_packet->setQos(found_category->second->id);
		...
		if(this->category_map.count(dscp_value))
                {
                        UTI_ERROR("Traffic category %ld - [%s] rejected: identifier "
                                  ...
                                  mac_queue_name.c_str(), this->category_map[dscp_value]->name.c_str());
                        return false;
                }

                category = new TrafficCategory();

                category->id = mac_queue_prio;
                category->name = mac_queue_name;
