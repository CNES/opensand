...
/**
 * @file UnitConverterFixedBitLength.h
 * @brief Converters for OpenSAND units
 * @author Julien Bernard <jbernard@toulouse.viveris.com>
 * @author Aurelien DELRIEU <adelrieu@toulouse.viveris.com>
 */

#ifndef _UNIT_CONVERTER_FIXED_BIT_LENGTH_H_
#define _UNIT_CONVERTER_FIXED_BIT_LENGTH_H_

#include "OpenSandCore.h"
#include "UnitConverter.h"

/**
 * @class UnitConverterFixedBitLength
 * @brief class managing unit conversion between kbits/s, cells per frame, etc
 */
class UnitConverterFixedBitLength: public UnitConverter
{
 protected:

	vol_b_t packet_length_b;        ///< Fixed packet length (in bits)
	float packet_length_b_inv;      ///< Inverse of fixed packet length (in bits-1)
 ...
 public:
	 ...
	 * @param  length_b     The fixed packet length in bits
	 */
	UnitConverterFixedBitLength(
		time_ms_t duration_ms,
		unsigned int efficiency,
		vol_b_t length_b);
	virtual ~UnitConverterFixedBitLength();
	 ...
	 * @param length_b  The fixed packet length in bits
	 */
	void setPacketBitLength(vol_b_t length_b);
