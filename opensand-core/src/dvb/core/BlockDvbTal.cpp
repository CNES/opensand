...
BlockDvbTal::BlockDvbTal(const std::string &name, dvb_specific specific) : Rt::Block<BlockDvbTal, dvb_specific>{name, specific},
																		   BlockDvb{},
...
bool Rt::DownwardChannel<BlockDvbTal>::onEvent(const MessageEvent &event)
{
	...
	// message and send it
	// TODO move in a dedicated class TODO TODO TODO
	// if(DownwardChannel<BlockDvbTal>::qos_server_sock != -1)
	// {
	...
	case EmulatedMessageType::BbFrame:
	{
		...
		if (burst)
		{
			...
			LOG(this->log_receive_channel, LEVEL_INFO,
				...
				nb_bursts, this->pkt_hdl->getName(),
