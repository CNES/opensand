...
SimpleRle::SimpleRle() : SimpleEncapPlugin(NET_PROTO::RLE,"RLE")
{
...
void SimpleRle::generateConfiguration(const std::string &, const std::string &, const std::string &)
{
...
bool SimpleRle::init(void)
{
		...
		LOG(this->log_simple, LEVEL_NOTICE,
			"ALPDU protection: %s\n", protection.c_str());
...
bool SimpleRle::decapNextPacket(Rt::Ptr<NetPacket> packet, std::vector<Rt::Ptr<NetPacket>> &decap_packets)
{
	bool success = false;
	...
	if (!SimpleRle::getLabel(packet->getPayload(), label))
	{
		LOG(this->log_simple, LEVEL_ERROR,
	...
	{
		LOG(this->log_simple, LEVEL_DEBUG, "Packet requiring a new rle receiver");
		...
		LOG(this->log_simple, LEVEL_DEBUG, "rle receiver created");
	}
	else
	{
		LOG(this->log_simple, LEVEL_DEBUG, "Packet requiring an existing rle receiver");
	...
	LOG(this->log_simple, LEVEL_DEBUG, "Initialize SDUs before rle decapsulation (max_count=%u, count=%u)",
...
Rt::Ptr<NetPacket> SimpleRle::build(const Rt::Data &data,
											 ...
											 uint8_t dst_tal_id)
{
	...
	return Rt::make_ptr<NetPacket>(data, data_length,
								   this->getName(), this->ether_type,
								   qos, src_tal_id, dst_tal_id, 0);
}

bool SimpleRle::encapNextPacket(Rt::Ptr<NetPacket> packet,
										 ...
										 Rt::Ptr<NetPacket> &remaining_data)
{
	...
	if (!SimpleRle::getLabel(*packet, label))
	{
		LOG(this->log_simple, LEVEL_ERROR,
	...
	if (it == this->transmitters.end())
	{
		...
		LOG(this->log_simple, LEVEL_DEBUG, "Packet requiring a new rle transmitter");
		...
		LOG(this->log_simple, LEVEL_DEBUG, "rle transmitter created");
	}
	else
	{

		LOG(this->log_simple, LEVEL_DEBUG, "Packet requiring an existing rle transmitter");
		// Get existing identifier and context
		delete identifier;
		identifier = it->first;
	...
	{
		LOG(this->log_simple, LEVEL_DEBUG, "rle encapsulation of this SDU (len=%u bytes)",
			packet->getTotalLength());
		if (0 < prev_queue_size)
		{
			LOG(this->log_simple, LEVEL_ERROR, "rle encapsulation already in progress (queue size=%u bytes)",
				prev_queue_size);
		}
	...
	LOG(this->log_simple, LEVEL_DEBUG, "rle fragmentation");
	ppdu_size = 0;
	LOG(this->log_simple, LEVEL_DEBUG,
...
encap_end:
	if ((pkt_it == sent_packets.end()) && partial_encap)
	{
		...
		LOG(this->log_simple, LEVEL_DEBUG, "Add packet to partially sent packets list");
	...
	else if ((pkt_it != sent_packets.end()) && !partial_encap)
	{
		...
		LOG(this->log_simple, LEVEL_DEBUG, "Remove packet from partially sent packets list");
...
bool SimpleRle::decapAllPackets(Rt::Ptr<NetContainer> encap_packets,
...
bool SimpleRle::getSrc(const Rt::Data &data, tal_id_t &tal_id) const
{
	uint8_t label[LABEL_SIZE];

	// Get label (this will success if it is the first fragment)
	if (!SimpleRle::getLabel(data, label))
	{
		return false;
	}
...
bool SimpleRle::getQos(const Rt::Data &data, qos_t &qos) const
{
	uint8_t label[LABEL_SIZE];
...
bool SimpleRle::getDst(const Rt::Data &data, tal_id_t &tal_id) const
{
	uint8_t label[LABEL_SIZE];
...
bool SimpleRle::getLabel(const NetPacket &packet, uint8_t label[])
{
...
bool SimpleRle::getLabel(const Rt::Data &data, uint8_t label[])
{
