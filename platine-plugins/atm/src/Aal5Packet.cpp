...
uint16_t Aal5Packet::getTotalLength()
{
	return this->data.length();
}

uint16_t Aal5Packet::getPayloadLength()
{
	// data must be at least 8 bytes long (= AAL5 trailer length)
	if(this->data.length() < 8)
	{
...
void Aal5Packet::setSrcTalId(uint8_t tal_id)
{
	this->src_tal_id = tal_id;
}

void Aal5Packet::setDstTalId(uint8_t tal_id)
{
	this->dst_tal_id = tal_id;
}

void Aal5Packet::setQos(uint8_t qos)
{
	this->qos = qos;
}
