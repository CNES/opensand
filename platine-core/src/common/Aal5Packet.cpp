...
Aal5Packet::Aal5Packet(unsigned char *data, unsigned int length):
	NetPacket(data, length)
{
	this->_name = "AAL5";
	this->_type = NET_PROTO_AAL5;
...
Aal5Packet::Aal5Packet(Data data): NetPacket(data)
{
	this->_name = "AAL5";
	this->_type = NET_PROTO_AAL5;
	this->_data.reserve(1500);
...
void Aal5Packet::setQos(int qos)
{
	this->_qos = qos;
}

unsigned long Aal5Packet::macId()
{
	return this->_macId;
}

void Aal5Packet::setMacId(unsigned long macId)
{
	this->_macId = macId;
}

long Aal5Packet::talId()
{
	return this->_talId;
}

void Aal5Packet::setTalId(long talId)
{
	this->_talId = talId;
}

bool Aal5Packet::isValid()
{
	...
	if((this->payloadLength() + 8) > (int) this->_data.length())
	{
		UTI_NOTICE("%s payload (%d) + trailer (8) > total length (%d)\n",
		           FUNCNAME, this->payloadLength(), this->_data.length());
		goto invalid;
	}

	// AAL5 packet length must be multiple of 48 (= ATM payload length)
	if(this->_data.length() % 48 != 0)
	{
		UTI_NOTICE("%s total length (%d) is not a multiple of 48\n",
		           FUNCNAME, this->_data.length());
		goto invalid;
	}

	// calculate the CRC
	crc = Aal5Packet::calcCrc(this->_data.substr(0, this->_data.length() - 4));
	cur_crc = this->crc();
...
uint16_t Aal5Packet::totalLength()
{
	return this->_data.length();
}

uint16_t Aal5Packet::payloadLength()
{
	// data must be at least 8 bytes long (= AAL5 trailer length)
	if(this->_data.length() < 8)
	{
		UTI_ERROR("[Aal5Packet::payloadLength] invalid AAL5 packet\n");
		return 0;
	}

	return (uint16_t)
	       (((this->_data.at(this->_data.length() - 6) & 0xff) << 8)
	       + ((this->_data.at(this->_data.length() - 5) & 0xff) << 0));
}

Data Aal5Packet::payload()
{
	if(!this->isValid())
	{
	...
	uint16_t payload_len = this->payloadLength();
	...
	return this->_data.substr(0, payload_len);
}
...
Aal5Packet *Aal5Packet::createFromPayload(Data payload)
{
	...
	if(last_atm_cell_len > 0)
		padding_len = AtmCell::length() - 5 - last_atm_cell_len;
	else
		padding_len = 0;
...
uint32_t Aal5Packet::crc()
{
	...
	return (uint32_t)
		(((this->_data.at(this->_data.length() - 4) & 0xff) << 24)
		 + ((this->_data.at(this->_data.length() - 3) & 0xff) << 16)
		 + ((this->_data.at(this->_data.length() - 2) & 0xff) << 8)
		 + ((this->_data.at(this->_data.length() - 1) & 0xff) << 0));
}

unsigned int Aal5Packet::nbAtmCells()
{
	return (this->_data.length() / 48);
}

Data Aal5Packet::atmCell(unsigned int index)
{
	return this->_data.substr(index * 48, 48);
}

// static
NetPacket *Aal5Packet::create(Data data)
{
	return new Aal5Packet(data);
}
